...
class AskChatbotUseCase {
  constructor(smartSearchUseCase, aiService) {
    this.smartSearchUseCase = smartSearchUseCase;
    this.aiService = aiService;
  }

  async execute(question) {
    const productsFound = await this.smartSearchUseCase.execute(question);

    const inventoryContext = productsFound
      .map((p) => `- Producto: ${p.name}, Precio: ${p.price}, Stock: ${p.stock}`)
      .join('\n');

    const prompt = chatbotPrompt({ inventoryContext, userQuestion: question });

    return await this.aiService.generateChatResponse(prompt);
  }
}

module.exports = AskChatbotUseCase;